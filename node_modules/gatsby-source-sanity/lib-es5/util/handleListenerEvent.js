...
var handleDrafts_1 = require("./handleDrafts");
...
function handleListenerEvent(event, publishedNodes, context, processingOptions) {
    ...
    var current = getNode(createNodeId(handleDrafts_1.unprefixId(event.documentId)));
    var published = publishedNodes.get(handleDrafts_1.unprefixId(event.documentId));
    if (published) {
        published = removeGatsbyInternalProps_1.removeGatsbyInternalProps(published);
    }
    var touchedIsDraft = handleDrafts_1.isDraftId(event.documentId);
    var currentIsDraft = current && handleDrafts_1.isDraftId(current._id);
    ...
    if (event.transition === 'disappear') {
        ...
        if (touchedIsDraft) {
            ...
            if (current && !currentIsDraft) {
                publishedNodes.set(handleDrafts_1.unprefixId(event.documentId), current);
            }
        }
            ...
            publishedNodes.set(event.documentId, normalize_1.processDocument(event.result, __assign({}, processingOptions, { skipCreate: true })));
        }
        else {
