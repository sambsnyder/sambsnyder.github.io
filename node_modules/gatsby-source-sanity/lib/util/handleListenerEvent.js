...
function handleListenerEvent(event, publishedNodes, context, processingOptions) {
    ...
    const current = getNode(createNodeId(handleDrafts_1.unprefixId(event.documentId)));
    let published = publishedNodes.get(handleDrafts_1.unprefixId(event.documentId));
    if (published) {
        published = removeGatsbyInternalProps_1.removeGatsbyInternalProps(published);
    }
    const touchedIsDraft = handleDrafts_1.isDraftId(event.documentId);
    const currentIsDraft = current && handleDrafts_1.isDraftId(current._id);
    ...
    if (event.transition === 'disappear') {
        ...
        if (touchedIsDraft) {
            ...
            publishedNodes.set(event.documentId, normalize_1.processDocument(event.result, Object.assign({}, processingOptions, { skipCreate: true })));
        }
        else {
