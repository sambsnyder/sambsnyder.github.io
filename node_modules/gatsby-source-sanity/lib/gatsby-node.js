...
exports.onPreBootstrap = async (context, pluginConfig) => {
    ...
    try {
        ...
        const graphqlSdl = await rewriteGraphQLSchema_1.rewriteGraphQLSchema(api, config);
        ...
        const typeMap = remoteGraphQLSchema_1.getTypeMapFromGraphQLSchema(api, pluginConfig);
...
function resolveReferences(obj, depth, maxDepth, context, pluginConfig) {
    ...
    if (typeof obj._ref === 'string') {
        const id = obj._ref;
        const node = getNode(createNodeId(overlayDrafts ? normalize_1.unprefixDraftId(id) : id));
